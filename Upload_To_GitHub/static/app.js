...
function updateSetupSummary() {
    ...
    const ready = !!geoState.setupConfirmed && tbs.length > 0;
    ...
    if (lockHint) {
        ...
        lockHint.textContent = ready ? '' : 'กรุณายืนยันพื้นที่รับผิดชอบด้านบนก่อน';
    }
}
...
function applyOfficeMeetingRulesToRecord(rec) {
    ...
    if (!rec.target_num) rec.target_num = 7;
    return true;
}
...
function uploadExcelFile() {
    if (!geoState.setupConfirmed || ![...getResponsibleTambonSet()].length) {
        alert('กรุณายืนยันตำบลที่รับผิดชอบใน Step 0 ก่อนอัปโหลด Excel');
...
async function generateAutoMonthPlanFromWebUI() {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        if (dayOfWeek === 1) {
            ...
            const randTarget = Math.floor(Math.random() * 6) + 8; // 8 - 13 people
