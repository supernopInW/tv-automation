...
1. กดปุ่ม **「Login T&V (เปิดเบราว์เซอร์)」** — ระบบเปิดหน้าต่าง Chromium ไปยังหน้า Login ของพอร์ทัล
2. Login ด้วยบัญชี T&V **ของตนเอง** ในหน้าต่างนั้น (ระบบไม่รับและไม่เก็บรหัสผ่าน; Session อยู่ใน browser profile บนเครื่อง local เท่านั้น)
3. รอป้ายสถานะเป็น **T&V: Logged In ✓** — ปุ่มเริ่ม Automation จะปลดล็อก
4. ระบุผู้อนุมัติให้ตรงรายชื่อในพอร์ทัล
5. เลือกโหมด:
   - **Dry-run** ก่อนเสมอเมื่อเป็นเดือนใหม่ / ไฟล์ใหม่
   - **บันทึกชั่วคราว** เมื่ออยากเก็บร่าง
   - **บันทึก & ส่ง** เมื่อตรวจครบแล้ว

> การกรอกอัตโนมัติใช้ได้เฉพาะเครื่อง local (headed browser) — เซิร์ฟเวอร์ headless เช่น Render จะปฏิเสธ `/api/run` อย่างชัดเจน

### สิ่งที่ระบบทำบนพอร์ทัล

1. ใช้หน้าต่างเบราว์เซอร์และ Session ที่ผู้ใช้ Login ไว้ (ตรวจ `is_tv_logged_in` ก่อนเริ่มและระหว่างทำงาน; ถ้า Session หมดอายุจะหยุดทันที)
2. เข้า Workflow 26
